function [x,histout,costdata] = GP_alg(f,x0,erro,S_D,up,low,tol,maxit)
%                                                   % args included !!
% C. T. Kelley, June 11, 1998
%
% This code comes with no guarantee or warranty of any kind.
%
% function [x,histout,costdata] = GP_alg / gradproj(x0,f,up,low,tol,maxit)
%
...
itc=1; 
[fc,gc]=feval(f,xc,erro,S_D);   % I changed this line!!
...
while(norm(pgc) > tol & itc <= maxit)
        lambda=1;
        xt=kk_proj(xc-lambda*gc,kku,kkl); ft=feval(f,xt,erro,S_D);
        numf=numf+1;
	iarm=0; itc=itc+1;
...
%
	...
	while(ft > fgoal)
		...
		xt=kk_proj(xc-lambda*gc,kku,kkl);
                pl=xc-xt;
		ft=feval(f,xt,erro,S_D); numf = numf+1;
		if(iarm > 15) % The original value was 10 !! I changed to 15 in order to prevent errors
		disp(' Armijo error in gradient projection')
                histout=ithist(1:itc,:); costdata=[numf, numg, numh];
		return; end
                fgoal=fc-(pl'*pl)*(alp/lambda);
	end
	xc=xt; [fc,gc]=feval(f,xc,erro,S_D); numf=numf+1; numg=numg+1;
        pgc=xc-kk_proj(xc-gc,kku,kkl); 
